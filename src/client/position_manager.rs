...
pub struct PositionManager {
    init_datas: HashMap<SocketAddr, watch::Receiver<Option<InitData>>>,
    pos_txs: HashMap<SocketAddr, watch::Sender<PosCoord>>,
}
...
impl PositionManager {
    ...
    pub fn register(&mut self, addr: SocketAddr) -> (watch::Sender<Option<InitData>>, watch::Receiver<PosCoord>) {
        ...
        self.init_datas.insert(addr, init_data_rx);
        self.pos_txs.insert(addr, pos_tx);
    ...
    pub async fn run(&mut self, server_addr: &str) {
        ...
        loop {
            ...
            if let Ok((_, client_addr)) = sock.recv_from(&mut buf).await {
                let Some(init_data) = self.init_datas.get(&client_addr).map(|x| *x.borrow()).flatten() else { continue; };
                ...
                self.pos_txs.get(&client_addr).map(|x| x.send(screen_pos).unwrap());
            }
        }
    }
}